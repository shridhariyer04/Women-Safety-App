...
export const EmergencyContactsForm = ({ userId, userName, onComplete }: EmergencyContactsFormProps) => {
  ...
  const validateContacts = () => {
    ...
    for (const contact of filledContacts) {
      if (!phoneRegex.test(contact.phone_number)) {
        Alert.alert('Error', `Invalid phone number for ${contact.contact_name}`);
        return false;
      }
    }
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.subtitle}>Please add at least one emergency contact for safety purposes</Text>
